...
def copy_terminology(connection, common_config, tenant_config):
...
def update_tenant_db():
    try:
        ...
        connection = psycopg2.connect(common_config['TerminologyDB']['connectionsString'])

        for tenant in tenants_config:
            tenant_config = tenant['tenantDB']['tenantInformation']
            keyvault_name = tenant['keycloak']['aad_tenant_id']
            ...
            copy_terminology(connection, common_config, tenant_config)
